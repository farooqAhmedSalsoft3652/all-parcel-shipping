...
import { mentorshipStatusFilter, SERVER_URL } from "@config/data";
...
const MentorShipReq = () => {
  usePageTitle("Mentorship Request");
  ...
  const loadMentorshipRequest = async () => {
    setLoad(true);
    let url = `/mentorship-request?page=${currentPage}`;
                          ...
                          <tr key={index}>
                            ...
                            <td>
                             <div className="d-flex justify-content-start">
                                <img className="ad_poster_img me-2" src={SERVER_URL + item.mentee_avatar} alt={true.toString()} />
                                <p className="flex-grow-1 align-self-center text-start mb-0">{item.mentee_name}</p>
                             </div>
                            </td>
                            <td>#{item.id}</td>
                            ...
                            <td>
                               <Link to={`/mentorship-request/details/${item.id}`} className="tableAction view text-decoration-underline">View</Link>
                            </td>
                          </tr>
                        )))}
                      </tbody>
