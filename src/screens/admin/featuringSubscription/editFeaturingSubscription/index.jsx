...
const EditFeaturingSubs = () => {
  ...
  const loadAdsDetail = async () => {
    setLoad2(true);
    let data = await axios.get(`/featured-management/${id}`)
      .then(response => {
  ...
  const handleClickUpdate = (e) => {
    ...
    let data = axios.post(`/featured-management/${id}/update`, formData)
      .then(response => {
        setShowModal(true);
        setLoad(false);
        setFormData({});
  ...
  const closeModal = () => {
    setShowModal(false);
    navigate("/admin/featuring-subscription");
  } 
...
export default EditFeaturingSubs;